...
import { Space, Avatar, Button, Form, Input, List, Image } from 'antd';
import { Comment } from '@ant-design/compatible';
import { FacebookOutlined, TwitterOutlined, InstagramOutlined, WeiboOutlined, UserOutlined } from '@ant-design/icons';
...
const CommentList = ({ comments }) => (
    <List
        dataSource={comments}
        header={<p style={{ color: 'white' }}>{ comments.length } { comments.length > 1 ? 'replies' : 'reply' }</p>}
        itemLayout='horizontal'
        renderItem={(props) => <Comment style={{ backgroundColor: 'black' }} 
                                        avatar={<Image src={avatar}></Image>} 
                                        datetime={<a style={{ color: 'lightblue' }}>{ moment('2016-11-22').fromNow() }</a>} 
                                        author={<a style={{ color: 'lightblue' }}>{ props.author }</a>}
                                        content={<p style={{ color: 'white', fontSize: 16 }}>{ props.content }</p>}
                                />}
    />
);
const Editor = ({ onSetUser, onComment, onSubmit, submitting, user, value }) => (
    <>
        <Form.Item>
            <TextArea rows={1} onChange={onSetUser} value={user} placeholder='Name'/>
        </Form.Item>
        <Form.Item>
            <TextArea rows={4} onChange={onComment} value={value} placeholder='Comment'/>
        </Form.Item>
        <Form.Item>
            <Button className='edit-button' htmlType='submit' loading={submitting} onClick={onSubmit} type='text'>
                Add Comment
            </Button>
        </Form.Item>
    </>
                            ...
                            <td valign='top' width={300} style={{ borderRight: '1px solid #292929' }}>
                                ...
                                <div>
                                    ...
                                    <Space style={{marginBottom: 20 }}>
                                        <InstagramOutlined style={{ color: 'white', fontSize: 40, marginLeft: 20, marginTop: 20 }}/>
                                        <FacebookOutlined style={{ color: 'white', fontSize: 40, marginLeft: 20, marginTop: 20 }}/>
                                        <TwitterOutlined style={{ color: 'white', fontSize: 40, marginLeft: 20, marginTop: 20 }}/>
                                        <WeiboOutlined style={{ color: 'white', fontSize: 40, marginLeft: 20, marginTop: 20 }}/>
                                    </Space>
                                </div>
                            </td>
